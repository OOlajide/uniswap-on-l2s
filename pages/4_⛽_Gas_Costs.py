...
@st.cache_data
def load_df2():
    df2 = pd.read_csv('dataset/df_2.csv')
    return df2
...
@st.cache_data
def load_df3():
    df3 = pd.read_csv('dataset/df_3.csv')
    return df3
...
@st.cache_data
def load_df24():
    df24 = pd.read_csv('dataset/df_24.csv')
    return df24
...
st.info("This page examines gas cost fluctuations within Uniswap, delving into variations in gas fees across different chains.", icon="ℹ️")
...
insight_2 = '<p style="font-family:sans-serif; color:#4d372c; font-size: 18px;">Among Layer 2s, Polygon stands out as the most economical, boasting the lowest average gas cost and the lowest average gas cost for swapping $1. This cost efficiency likely contributes to Polygon\'s dominance, evident in its significantly higher number of active pools compared to other chains. On the flip side, Optimism and Avalanche emerge as the costliest chains with an average gas cost of $0.5 each.</p>'
