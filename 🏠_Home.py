...
text_2 = '<p style="font-family:sans-serif; color:#4d372c; font-size: 20px;">This dashboard examines Uniswap\'s utilization across multiple blockchains. While our primary focus is on L2s: Arbitrum, Base, and Optimism. To provide a more comprehensive view, we\'ve expanded our analysis to include Avalanche, BSC, and Polygon. While these latter three are not L2s—Avalanche and BSC are separate Layer 1 blockchains, and Polygon is a sidechain—they are included due to Uniswap\'s significant presence on these chains. This broader scope enables us to assess Uniswap\'s performance across multiple chains, uncovering common trends and key insights that shape its adoption and usage.</p>'
...
@st.cache_data
def load_df27():
    df27 = pd.read_csv('dataset/df_27.csv')
    return df27
